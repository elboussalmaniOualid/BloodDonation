...
public interface stockRepository extends CrudRepository<paquet,Integer> {
    ...
    @Query(value="select count(id) from paquet WHERE type='globules rouges' AND date_of_donation >= (CURDATE() - INTERVAL :numberOfDays DAY )", nativeQuery = true )
    public int ReserveOfglobules(@Param("numberOfDays") int numberOfDays);
